resource "aws_acm_certificate" "website_cert" {
  ...
  subject_alternative_names = ["www.it-is-what-it-is.site"]
  tags = {
...
resource "aws_cloudfront_distribution" "project_website" {
  provider = aws.us-east-1
  origin {
    domain_name = aws_s3_bucket.project-webpage-bucket.bucket_domain_name
...
resource "aws_route53_record" "website_dns" {
  zone_id = aws_route53_zone.project_zone.zone_id
  name    = "www.it-is-what-it-is.site"
  type    = "A"
  alias {
